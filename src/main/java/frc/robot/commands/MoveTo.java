...
public class MoveTo extends Command {
    ...
    public void initialize() {
        ...
        target_Rotations = Rotations.of(-((drive.getLeftRotations().in(Rotations) + drive.getRightRotations().in(Rotations)) / 2) + ((Math.sqrt(Math.pow(x.in(Meters), 2) + Math.pow(y.in(Meters), 2))) / Constants.WHEEL_CIRCUMFERENCE.in(Meters)));
